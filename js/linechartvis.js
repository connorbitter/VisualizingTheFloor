...
LineChartVis = function(_parentElement, _data, _stat, _points){
...
LineChartVis.prototype.initVis = function(){
	...
	vis.extent = d3.extent(vis.data, function(d){return d[vis.stat]});
...
LineChartVis.prototype.initData = function(){
	var vis = this;
	...
	for (i = vis.extent[0]; i < vis.extent[1] + 0.1; i += 0.1) {
		...
		for (j = 0; j < vis.filteredData.length; j++) {
			if (vis.filteredData[j][vis.stat] == i_r) {
				shots_total++;
...
LineChartVis.prototype.onSelectionChange = function(){
	var vis = this;
